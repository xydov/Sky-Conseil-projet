...
def renommer_colonne_A330(df):
    """
    Renomme les colonnes d'un DataFrame pour correspondre aux attributs spécifiés pour les données A330.
    ...
    correspondance = {
        ...
        'Standard altitude': 'Altitude',
        'Fuel flow Eng 1 (KG)': 'Fuel_E1',
        'Fuel flow Eng 2 (KG)': 'Fuel_E2',
        'Total Fuel (derived)': 'Total_Fuel',
        'Center of gravity': 'Center_Of_Gravity',
        'A/C Gross Weight (KG)': 'Gross_Weight',
        'Static Air Temperature': 'Static_Air_Temperature',
        ...
        'Throttle Resolver angle Eng 1': 'Throttle_Angle_E1',
        'Throttle Resolver angle Eng 2': 'Throttle_Angle_E2',
        'Flex temp': 'Flex_Temp',
        'APU Fuel flow (KG)': 'APU_Fuel_flow',
        'APU Master SW ON': 'APU_Master_SW_ON',
        'Pack cont. 1 P/B Off': 'Pack_Cont_1',
        'Pack cont. 2 P/B Off': 'Pack_Cont_2',
        'Gear Selector Up': 'Gear_Selector_Up',
        'Gear Selector Down': 'Gear_Selector_Down',
        'LDG Squat Switch-Nose': 'L_G_S_S_N',
        'Flap Handle Gated (derived)': 'Flap_Handle',
        'Flap Surface Angle': 'Flap_Angle',
        'Eng 1 THR REV fully deployed': 'Eng1_THR_REV_fully_deployed',
        'Eng 2 THR REV fully deployed': 'Eng2_THR_REV_fully_deployed',
        'Speed brake selection': 'Speed_Brake',
...
def nettoyage_reecriture_A330(df):
    """
    Fonction nettoyage_reecriture_A330 :
    ...
    iii. Nettoyage des colonnes 'Gear_Selector_Up' et 'Gear_Selector_Down' de manière similaire.
    iv. Suppression des lignes où la valeur dans la colonne 'Land Gear Squat Switch - Nose' est égale à 1.
    A n'appeler que sur A330
    ...
    df['Gear_Selector_Up'] = df['Gear_Selector_Up'].str.extract('(\d+)')
    df['Gear_Selector_Down'] = df['Gear_Selector_Down'].str.extract('(\d+)')

    # Convertir les colonnes en type int
    df['L_G_S_S_N'] = df['L_G_S_S_N'].astype('Int64')
    df['Gear_Selector_Up'] = df['Gear_Selector_Up'].astype('Int64')
    df['Gear_Selector_Down'] = df['Gear_Selector_Down'].astype('Int64')

    valeur_a_supprimer = 1

    # Suppression des lignes où la valeur dans la colonne 'Land Gear Squat Switch - Nose' est égale à 1
...
def supprimer_colonnes_A330(df):
    """
    Supprime les colonnes spécifiées pour le type d'avion A330.
    ...
    """
    colonnes_a_supprimer_A330 = [
        'City Pair To', 'City Pair From', 'Flight Number', 'DATE  Day', 'DATE  Month', 'UTC Hours', 'UTC Minutes',
        'UTC Seconds', 'Ground Speed', 'Alt Baro Set Capt', 'CAPT Present Position Latitude',
        'CAPT Present Position Longitude', 'True Heading', 'Wind Direction True', 'Wind Speed',
        'APU Fuel flow (KG)', 'APU Master SW ON', 'Eng 1 THR REV fully deployed', 'Eng 2 THR REV fully deployed',
        'Fuel_E1', 'Fuel_E2'
    ]
    return df.drop(columns=colonnes_a_supprimer_A330, inplace=False, errors='ignore')


def gestion_phases_A330(df):
    """
    Applique le traitement de gestion des phases sur un DataFrame contenant des données de vol de l'avion A330.
    ...
    if max(df["Time_secs"]) > 35000:
        seuil = df['Altitude'].quantile(0.35)
    elif max(df["Time_secs"]) > 30000:
        seuil = df['Altitude'].quantile(0.38)
    elif max(df["Time_secs"]) > 20000:
        seuil = df['Altitude'].quantile(0.35)
...
df_A330_modif  = pd.read_csv(csv_file_path)


df_A330_modif=remplacer_etoile_par_vide(df_A330_modif)
df_A330_modif=clean_and_fill_dataframe(df_A330_modif)

df_A330_modif=renommer_colonne_A330(df_A330_modif)



df_A330_modif= nettoyer_tail_number(df_A330_modif)
df_A330_modif=nettoyage_reecriture_A330(df_A330_modif)


for column in ['Roll_Angle']:
  df_A330_modif[column] = df_A330_modif[column].apply(extract_value_L_R)
df_A330_modif= transformer_colonnes_aeronef(df_A330_modif,'A330')
for col in colonne_U_D_A330:
  df_A330_modif[col] = df_A330_modif[col].apply(extract_value_U_D)

df_A330_modif=supprimer_colonnes_A330(df_A330_modif)

df_A330_modif = convert_columns_to_numeric(df_A330_modif, numeric_columns_A330)


df_A330_modif=gestion_phases_A330(df_A330_modif)
df_A330_modif=seasonal_decomposition_detection(df_A330_modif, 'Gross_Weight', period=365, model='additive', sensitivity=3, local_window_size=500)
df_A330_modif=fill_missing_vertical_speed(df_A330_modif)
