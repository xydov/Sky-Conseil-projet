...
def renommer_colonne_A321_Type_Inconnu(df):
    """
    Renomme les colonnes d'un DataFrame pour correspondre aux attributs spécifiés pour les données A321 de type inconnu.
    ...
    correspondance = {
        ...
        'Speed Brake Command': 'Speed_Brake',
        'Fuel': 'Fuel','Vitesse_Verticale': 'Vertical_Speed', 'Montée': 'Climb', 'Descente': 'Descent','Fuel':'Instant_Fuel'
    }
...
def supprimer_colonnes_A321_Type_Inconnu(df):
    """
    Supprime les colonnes spécifiées pour le type d'avion A321 de type inconnu.
    ...
    """
    colonnes_a_supprimer_A321_Type_Inconnu = [
        ...
        'Reverser Deployed Eng 1', 'Reverser Deployed Eng 2', 'Fuel_E1', 'Fuel_E2'
    ]
    return df.drop(columns=colonnes_a_supprimer_A321_Type_Inconnu, inplace=False, errors='ignore')


def gestion_phases_Type_Inconnu(df):
    """
    Applique le traitement de gestion des phases sur un DataFrame contenant des données de vol de type inconnu.
    ...
    if max(df["Time_secs"]) > 15000:
        seuil = df['Altitude'].quantile(0.24)
    elif max(df["Time_secs"]) > 14000:
        seuil = df['Altitude'].quantile(0.33)
    elif max(df["Time_secs"]) > 12000:
        seuil = df['Altitude'].quantile(0.4)
    elif max(df["Time_secs"]) > 8000:
        seuil = df['Altitude'].quantile(0.45)
    elif max(df["Time_secs"]) < 3000:
        seuil = df['Altitude'].quantile(0.975)
    elif max(df["Time_secs"]) < 3100:
        seuil = df['Altitude'].quantile(0.967)
    elif max(df["Time_secs"]) < 3500:
        seuil = df['Altitude'].quantile(0.95)
    elif max(df["Time_secs"]) > 4000:
        seuil = df['Altitude'].quantile(0.7)
...
df_Type_Inconnu_modif  = pd.read_csv(csv_file_path)


df_Type_Inconnu_modif=remplacer_etoile_par_vide(df_Type_Inconnu_modif)
df_Type_Inconnu_modif=clean_and_fill_dataframe(df_Type_Inconnu_modif)

df_Type_Inconnu_modif=renommer_colonne_A321_Type_Inconnu(df_Type_Inconnu_modif)



df_Type_Inconnu_modif=nettoyage_reecriture_A319_A321_Type_Inconnu(df_Type_Inconnu_modif)
df_Type_Inconnu_modif=traitement_gear_left_A319_A321_Type_Inconnu(df_Type_Inconnu_modif)


for column in ['Roll_Angle']:
  df_Type_Inconnu_modif[column] = df_Type_Inconnu_modif[column].apply(extract_value_L_R)
df_Type_Inconnu_modif= transformer_colonnes_aeronef(df_Type_Inconnu_modif,'Type_Inconnu')
for col in colonne_U_D_A321_Type_Inconnu_A319:
  df_Type_Inconnu_modif[col] = df_Type_Inconnu_modif[col].apply(extract_value_U_D)

df_Type_Inconnu_modif=supprimer_colonnes_A321_Type_Inconnu(df_Type_Inconnu_modif)

df_Type_Inconnu_modif = convert_columns_to_numeric(df_Type_Inconnu_modif, numeric_columns_A319)

print(df_Type_Inconnu_modif.info())
df_Type_Inconnu_modif=gestion_phases_Type_Inconnu(df_Type_Inconnu_modif)
df_Type_Inconnu_modif=seasonal_decomposition_detection(df_Type_Inconnu_modif, 'Gross_Weight', period=365, model='additive', sensitivity=3, local_window_size=500)
df_Type_Inconnu_modif=fill_missing_vertical_speed(df_Type_Inconnu_modif)
