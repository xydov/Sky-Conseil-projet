...
def gestion_phases_A321(df):
    """
    Applique le traitement de gestion des phases sur un DataFrame contenant des données de vol de l'avion A321.
    ...
    if max(df["Time_secs"]) > 10000:
        seuil = df['Altitude'].quantile(0.4)
    elif max(df["Time_secs"]) < 4000:
        seuil = df['Altitude'].quantile(0.9)
    elif max(df["Time_secs"]) > 8000:
        seuil = df['Altitude'].quantile(0.5)
    elif max(df["Time_secs"]) > 4000:
        seuil = df['Altitude'].quantile(0.6)
    ...
    for index, row in df.iterrows():
        t_air_temp = float(row["T_Air_Temp"])  # Convert to float
        mach = row["Mach"]
...
df_A321_modif  = pd.read_csv(csv_file_path)



df_A321_modif=remplacer_etoile_par_vide(df_A321_modif)
df_A321_modif=clean_and_fill_dataframe(df_A321_modif)

df_A321_modif=renommer_colonne_A321_Type_Inconnu(df_A321_modif)



df_A321_modif= nettoyer_tail_number(df_A321_modif)
df_A321_modif=nettoyage_reecriture_A319_A321_Type_Inconnu(df_A321_modif)
df_A321_modif=traitement_gear_left_A319_A321_Type_Inconnu(df_A321_modif)


for column in ['Roll_Angle']:
  df_A321_modif[column] = df_A321_modif[column].apply(extract_value_L_R)
df_A321_modif= transformer_colonnes_aeronef(df_A321_modif,'A321')
for col in colonne_U_D_A321_Type_Inconnu_A319:
  df_A321_modif[col] = df_A321_modif[col].apply(extract_value_U_D)

df_A321_modif=supprimer_colonnes_A321_Type_Inconnu(df_A321_modif)

df_A321_modif = convert_columns_to_numeric(df_A321_modif, numeric_columns_A319)


df_A321_modif=gestion_phases_A321(df_A321_modif)
df_A321_modif=seasonal_decomposition_detection(df_A321_modif, 'Gross_Weight', period=365, model='additive', sensitivity=3, local_window_size=500)
df_A321_modif=fill_missing_vertical_speed(df_A321_modif)
